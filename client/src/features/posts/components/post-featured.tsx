    ...
    return (
        <div className="relative group rounded-3xl border bg-card overflow-hidden transition-all">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6">
                ...
                <Link to={`/post/${post._id}`} className="block group-hover:text-primary transition-colors">
                    <h3 className="text-2xl font-bold mb-2">{post.title}</h3>
                    <p className="text-muted-foreground line-clamp-2">{stripHtmlAndTruncate(post.content)}</p>
                </Link>
                {post.author && (
